...
public final class ProtectedStateStore {
    ...
    /// Clears ONLY the pending reservation for a provably read-only operation.
    /// The committed version is never decremented or rolled back.
    public func release(version: UInt64) throws {
        var anchor = try checkedAnchor()
        guard anchor.pendingVersion == version, anchor.committedVersion + 1 == version else {
